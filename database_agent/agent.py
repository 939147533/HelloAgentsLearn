...
DATABASE_AGENT_PROMPT = """你是一个专业的数据库查询助手。你可以理解用户的自然语言查询，将其转换为SQL语句，并从Oracle数据库中获取数据。
...
class DatabaseAgent(ReActAgent):
    ...
    def _get_schema(self, input_text: str) -> str:
        ...
        oracle_tool = OracleQueryTool(self.db_config)
    ...
    def _generate_sql(self, input_text: str) -> str:
        ...
        if not self.schema_cache:
            oracle_tool = OracleQueryTool(self.db_config)
            self.schema_cache = oracle_tool.get_schema_info()
        
        sql_generator = SQLGeneratorTool(self.llm)
        sql = sql_generator.generate_sql(input_text, self.schema_cache)
    ...
    def _execute_query(self, input_text: str) -> str:
        ...
        oracle_tool = OracleQueryTool(self.db_config)
        result = oracle_tool.execute_query(sql)
